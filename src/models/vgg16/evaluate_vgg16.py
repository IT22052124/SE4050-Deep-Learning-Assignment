...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate VGG16 for Brain Tumor classification")
    parser.add_argument(
        "--data_dir",
        type=str,
...
def main():
    ...
    print(f"📊 Starting VGG16 evaluation...")
    ...
    with open(os.path.join(args.results_dir, "classification_report.txt"), "w") as f:
        f.write("Classification Report - VGG16\n")
        f.write("="*60 + "\n")
        f.write(report)
    ...
    plt.suptitle('VGG16 - Sample Predictions', fontsize=16, fontweight='bold')
    plt.tight_layout()
